...
export const userRouter = router({
  editName: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      console.log(
        new Date(Date.now() - 1000 * 60 * 60 * 24 * (Number(process.env.USER_NAME_CHANGE_COOLDOWN_DAYS) || 60))
      );
      ...
      const lastChange = await ctx.prisma.user.findFirst({
        where: {
          ...
          nameWasLastChangedAt: {
            gte: new Date(Date.now() - 1000 * 60 * 60 * 24 * (Number(process.env.USER_NAME_CHANGE_COOLDOWN_DAYS) || 60))
          }
        }
      });
      ...
      if (lastChange !== null) {
        throw new Error("O nome foi alterado nos últimos 60 dias. Por favor, tente novamente mais tarde.");
      }
      ...
      if (!/^(?=.{3,20}$)[a-zA-Z][a-zA-Z0-9_.^]*(?: [a-zA-Z0-9]+)*$/.test(input.newName)) {
        throw new Error("O nome não está de acordo com nossa políticas de nomes válidos.");
      }
  ...
  editBirthdate: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.newBirthdate >= dateToCheck) {
        throw new Error("A data de nascimento não é válida. Você precisa ter 10 anos ou mais.");
      }
